...
async fn record_emits_event() {
    ...
    let receipt = tx.send().await.expect("record").get_receipt().await.expect("receipt");
    ...
    let record_log = logs.iter().find(|l| l.address() == addr).expect("Recorded log");
    eprintln!("Recorded event: topic1(digest)={:?}", record_log.topics().get(1));
}
