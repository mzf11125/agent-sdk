...
//
// The L4 hash fields are sha256-based (NOT keccak256): the layer anchors
// off-chain (Nostr-relay-published) verdicts as well as on-chain ones,
// matching invinoveritas's reference implementation — see
// ComputeRawProposalHash / ComputeVerdictHash in recompute.go.
type JudgmentExecutionAttestation struct {
	...
	RawProposalHash    common.Hash    // sha256(canonical proposed-action artifact, pre-review)
	VerdictHash        common.Hash    // sha256(JCS({preimage fields})); binds the verdict to rawProposalHash (anti-verdict-shopping)
	ExecutedActionHash common.Hash    // sha256(canonical executed-action record), revealed at settlement
