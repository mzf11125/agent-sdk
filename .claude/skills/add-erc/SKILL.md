...
9. **Implement Layer 1 (contract wrappers).**
   ...
   - **Rust integration tests** (for ERCs with a contract interface):
     ...
     * Dev-dependencies needed in `rust/core/Cargo.toml`: `alloy = { version = "2", features = ["full"] }`, `tokio` (with `rt` and `macros` features). Version 2.x avoids serde conflicts present in 0.11/0.12. Update `rust/providers/Cargo.toml` to use the same alloy version so workspace resolution doesn't conflict.
     ...
     * Signer: use `ANVIL_KEY_0` from the anvil accounts file (`testkit/.anvil-accounts.json`) — this is the pre-funded deployer account.
